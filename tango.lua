    ...
    return setmetatable( 
      ...
      {options = options,
       ...
       throw_send_error =
         function(self,err)                           
           if err == 'timeout' then
             throw_comtimeout_error(self.path,'socket.send')
           end                           
           self:throw({code = send_error,
                       desc = 'tango socket.send error:'..err},2)
         end,
       throw_comtimeout_error =
         function(self,method)
           self:throw({code = comtimeout_error,
                       desc = 'tango comtimeout error during '..method},3)
         end,            
       throw_calltimeout_error =
         function(self)
           self:throw({code = calltimeout_error,
                       desc = 'tango calltimeout during call to:'..self.path},2)
         end,
       throw_protocol_error=
         function(self,err)    
           self:throw({code = protocol_error,
                       desc = 'tango protocol error:'..err},2)
         end,
       throw_receive_error= 
         function(self,err)                           
           if err == 'timeout' then
             self:throw_comtimeout_error('socket.receive')
           end
           self:throw({code = receive_error,
                       desc = 'tango socket.receive error:'..err},2)
         end},
      -- proxy metatable
      {
        --- Private helper.
        ...
        __call=
          function(self,...)
            ...
            if not responselen then
              if err == 'timeout' then
                self:throw_calltimeout_error()
              else
                self:throw_receive_error(err)
              end
            end                         
            -- convert ascii len to number of bytes
            responselen = tonumber(responselen)
            if not responselen then
              self:throw_protocol_error('length as ascii number string not ok')
            end                       
            if comtimeout ~= calltimeout then
              socket:settimeout(comtimeout)
            end
